...
float distanceToFront = 5; // Расстояние от центра танка до фар по оси Z
float distanceToSide = 2; // Расстояние от центра танка до фар по оси X
...
{   
    ...
    // Уменьшаем скорость, если зажата клавиша Control
    if (ctrlPressed) {
        adjustedMoveSpeed /= 3;
    }
    // Увеличиваем скорость, если зажата клавиша Shift (и не зажата Control)
    ...
    {
        auto rad_angle = glm::radians(rotationAngle);
        characterPosition.x = characterPosition.x - adjustedMoveSpeed * cos(rad_angle);
        characterPosition.z = characterPosition.z - adjustedMoveSpeed * sin(rad_angle);

        auto x = characterPosition.x + distanceToCamera * cos(rad_angle);
        auto z = characterPosition.z + distanceToCamera * sin(rad_angle);
        CamPosition.x = x;
        CamPosition.z = z;

        rad_angle = glm::radians(rotationAngle + 180);
        glm::vec3 forward(cos(rad_angle), 0, sin(rad_angle));
        glm::vec3 left(-forward.z, 0, forward.x);


        left_headlight.position = characterPosition + forward * distanceToFront + left * distanceToSide;
        left_headlight.direction = glm::vec3(cos(glm::radians(rotationAngle + 180)), 0, sin(glm::radians(rotationAngle + 180)));

        right_headlight.position = characterPosition + forward * distanceToFront - left * distanceToSide;
    ...
    if (key == GLFW_KEY_SPACE && action == GLFW_PRESS) {
        glm::vec3 tankDirection = glm::vec3(cos(glm::radians(rotationAngle + 180)), 0, sin(glm::radians(rotationAngle + 180))); // Направление танка
        glm::vec3 tankPosition = characterPosition; // Позиция танка
        float projectileSpeed = 1.0f; // Скорость снаряда
        float maxDistance = 100.0f; // Максимальное расстояние

        projectiles.emplace_back(tankPosition, tankDirection, projectileSpeed, maxDistance);
    }
    if (key == GLFW_KEY_EQUAL && (action == GLFW_REPEAT || action == GLFW_PRESS))
    {
        //characterPosition.z += CharacterSpeed;
...
void genCubeTopFace(std::vector<GLuint>& outIndices, std::vector<glm::vec3>& outVertices,
    std::vector<glm::vec2>& outTextures, std::vector<glm::vec3>& outNormals,
    float size) {
    // Предполагаем, что центр куба находится в (0, 0, 0) и его высота равна size
    ...
    // Верхняя грань куба
    outVertices = {
        glm::vec3(-halfSize, halfSize, halfSize),  // Левый верхний угол
        glm::vec3(halfSize, halfSize, halfSize),   // Правый верхний угол
        glm::vec3(halfSize, halfSize, -halfSize),  // Правый нижний угол
        glm::vec3(-halfSize, halfSize, -halfSize)  // Левый нижний угол
    };
    ...
    // Нормали
    outNormals = { normal, normal, normal, normal };

    // Индексы (два треугольника)
